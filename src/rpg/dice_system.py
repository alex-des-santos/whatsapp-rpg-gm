...
class DiceSystem:
    ...
    def roll(self, expression: str, advantage: AdvantageType = AdvantageType.NORMAL) -> DiceRoll:
        """
        ...
        try:
            ...
            for match in dice_matches:
                ...
                if die_size not in [4, 6, 8, 10, 12, 20, 100]:
                    raise ValueError("Tipo de dado inválido. Use d4, d6, d8, d10, d12, d20 ou d100")
            ...
            if len(all_rolls) == 1 and dice_matches[0][1] == '20' and advantage != AdvantageType.NORMAL:
            ...
            if len(dice_matches) == 1 and dice_matches[0][1] == '20' and len(all_rolls) >= 1:
                highest_roll = max(all_rolls) if advantage == AdvantageType.ADVANTAGE else all_rolls[0]
                lowest_roll = min(all_rolls) if advantage == AdvantageType.DISADVANTAGE else all_rolls[0]
    ...
    def roll_attack(self, attack_bonus: int, advantage: AdvantageType = AdvantageType.NORMAL) -> DiceRoll:
        """Rolar ataque"""
    ...
    def roll_save(self, save_bonus: int, dc: int, advantage: AdvantageType = AdvantageType.NORMAL) -> Tuple[DiceRoll, bool]:
        """
    ...
    def roll_ability_check(self, ability_modifier: int, proficiency_bonus: int = 0, 
                          advantage: AdvantageType = AdvantageType.NORMAL) -> DiceRoll:
        """Rolar teste de habilidade"""
        total_bonus = ability_modifier + proficiency_bonus
        return self.roll(f"1d20{total_bonus:+d}", advantage)

    def roll_skill_check(self, skill_modifier: int, advantage: AdvantageType = AdvantageType.NORMAL) -> DiceRoll:
...
def calculate_ac(base_ac: int, dex_modifier: int, armor_bonus: int = 0, shield_bonus: int = 0) -> int:
    """Calcular Classe de Armadura"""
    return base_ac + dex_modifier + armor_bonus + shield_bonus

def calculate_spell_save_dc(spellcasting_ability_modifier: int, proficiency_bonus: int) -> int:
    """Calcular DC de resistência de magias"""
    return 8 + spellcasting_ability_modifier + proficiency_bonus

def calculate_spell_attack_bonus(spellcasting_ability_modifier: int, proficiency_bonus: int) -> int:
